...
| Schedulability (single CPU) | `sum_i (s_i / p_i) <= 1` | VM set is admissible iff condition holds |
| Pattern repetition window | `LCM(p_1, p_2, ..., p_n)` | Schedule repeats every least common multiple |
...
| SEDF schedulability | `sum_i (s_i / p_i) <= 1` |
| Hyperperiod | `LCM(p_1, ..., p_n)` |
| Credit share | `share_i = w_i / sum_j w_j` |
| Credit amount | `C_i = C_total * w_i / sum_j w_j` |
